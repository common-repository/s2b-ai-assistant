...
if (!class_exists('S2bAia_ChatBotConversationModel')) {
    ...
    class S2bAia_ChatBotConversationModel {
        ...
        private function _updChat($chat_hash = '', $message = '', $options = [], $author = '', $exp_time = 0){
            ...
            if(is_multisite()){
                set_site_transient('s2baia_'.$chat_hash, $values, $expiration);
            }else{
                set_transient('s2baia_'.$chat_hash, $values, $expiration);
            }
            return $chat_hash;
        ...
        public function deleteChat($chat_hash = ''){
            if(is_multisite()){
                return delete_site_transient('s2baia_'.$chat_hash);
            }else{
                return delete_transient('s2baia_'.$chat_hash);
            }
        }
        
        public function getChat($chat_hash = ''){
            if(is_multisite()){
                return get_site_transient('s2baia_'.$chat_hash);
            }else{
                return get_transient('s2baia_'.$chat_hash);
            }
