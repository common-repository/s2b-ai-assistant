...
if (!class_exists('S2bAia_BaseController')) {

    class S2bAia_BaseController {
        ...
        public function load_view($name, $data) {
            $path = S2BAIA_PATH . "/views/" . $name . ".php";
            $this->view_vars = $data;
            if (file_exists($path)) {
        ...
        public function load_model($name) {
            $path = S2BAIA_PATH . "/lib/models/" . $name . ".php";
            if (file_exists($path)) {
                $model_name = S2BAIA_CLASS_PREFIX . ucfirst($name);
                if (!class_exists(S2BAIA_CLASS_PREFIX.ucfirst($name))) {
                    include_once $path;
                }
        ...
        function verifyPostRequest($r, $nonce = '', $action = '') {
            ...
            if (!isset($_POST)) {
                $r['result'] = 1;
                $r['msg'] = __('Invalid request','s2b-ai-aiassistant');
                return $r;
            }
